...
// Explicitly configure MapLibre worker URL for production bundle compatibility
...
export function LiveMap({ gps, filtered, imu }) {
  ...
  useEffect(() => {
    ...
    const isLastKnown = Boolean(gps?.position_source === 'last_known');
    const hasLiveFix = isValidCoordinate && gps?.position_source === 'live' && gps?.live_fix_valid && !gps?.is_stale;
    ...
    const ensureMarker = (coords, colorSatCount, rotation, isImuHeading, isCached = false) => {
      try {
        if (!markerRef.current) {
          const markerEl = createASVMarkerElement(colorSatCount, isImuHeading, isCached);
        ...
        if (host) {
          const refreshed = createASVMarkerElement(colorSatCount, isImuHeading, isCached);
    ...
    if (hasLiveFix) {
      ...
      ensureMarker(coords, satCount, heading, imuHeadingValid, false);
      if (followVehicle) {
        try {
          map.easeTo({ center: coords, duration: 400 });
        } catch (e) {}
      }
    } else if (isValidCoordinate && isLastKnown) {
      const coords = [targetLon, targetLat];
      setLastValidPos(coords);
      ensureMarker(coords, 0, heading, imuHeadingValid, true);
      // Auto-center map on last-known position upon initial boot/load
      ...
      // Retain last known position with 0 satellites (amber/red) indicating lost fix
      ensureMarker(lastValidPos, 0, heading, imuHeadingValid, true);
    }
  }, [
