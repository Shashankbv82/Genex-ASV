import { Radio, Clock, CheckCircle, AlertCircle, Database } from 'lucide-react';
import { formatAge, formatLastKnownAge, getFixQualityLabel, getSatelliteMarkerColor, formatNumber } from '../../utils/formatters';
...
export function MapBottomBar({ gps }) {
  ...
  const color = isLastKnown ? '#F59E0B' : getSatelliteMarkerColor(satUsed);
    ...
    <div className="absolute bottom-3 left-3 right-3 z-10 bg-slate-950/85 backdrop-blur-md border border-slate-800/90 rounded-xl px-4 py-2.5 shadow-2xl flex flex-wrap items-center justify-between gap-3 text-xs">
      ...
      <div className="flex items-center gap-2">
        ...
        <div>
          ...
          <span className={`font-semibold ${isLive ? 'text-emerald-400' : isLastKnown ? 'text-amber-300' : 'text-slate-200'}`}>
            {isLive 
              ? getFixQualityLabel(gps?.fix_quality)
              : isLastKnown
              ? `LAST KNOWN FIX (${formatLastKnownAge(gps?.last_known_fix_age_seconds)})`
              : gps?.is_stale ? 'GPS Signal Stale' : 'No Valid Fix'}
          </span>
        </div>
      </div>
