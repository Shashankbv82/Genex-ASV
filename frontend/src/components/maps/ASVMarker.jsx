...
export function createASVMarkerElement(satellitesCount, headingValid = false, isLastKnown = false) {
  const baseColor = getSatelliteMarkerColor(satellitesCount);
  const color = isLastKnown ? '#F59E0B' : baseColor;
  const strokeColor = isLastKnown ? '#F59E0B' : (headingValid ? '#38BDF8' : color);
  const strokeDash = isLastKnown ? 'stroke-dasharray="3 2"' : '';
  ...
  el.innerHTML = `
    <!-- Pulse radar ring -->
    <div style="position: absolute; width: 44px; height: 44px; border-radius: 50%; background: ${color}; opacity: ${isLastKnown ? '0.15' : '0.20'}; animation: ping ${isLastKnown ? '3.5s' : '2.5s'} cubic-bezier(0, 0, 0.2, 1) infinite; ${isLastKnown ? 'border: 1px dashed #F59E0B;' : ''}"></div>
    ...
    <div style="position: relative; width: 32px; height: 38px; display: flex; align-items: center; justify-content: center; filter: drop-shadow(0 0 8px ${color}80);">
      <svg width="32" height="38" viewBox="0 0 32 38" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <path d="M16 2 C23 9, 27 18, 27 30 C27 34, 23 35, 16 35 C9 35, 5 34, 5 30 C5 18, 9 9, 16 2 Z" 
              fill="#0B111E" stroke="${color}" stroke-width="2" ${strokeDash} stroke-linejoin="round"/>
        ...
        <!-- Bow Tip Dot -->
        <circle cx="16" cy="4" r="1.5" fill="${isLastKnown ? '#F59E0B' : '#38BDF8'}"/>
      </svg>
    </div>
  `;
