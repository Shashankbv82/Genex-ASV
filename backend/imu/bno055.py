...
class BNO055:
    ...
    def _read_bytes(self, reg: int, length: int) -> bytes:
        """Reads multiple bytes from register with clock-stretching retry logic under shared I2C lock."""
        if self._fd is None:
    ...
    def _write_byte(self, reg: int, value: int):
        """Writes single byte to register with retry logic under shared I2C lock."""
        if self._fd is None:
            raise OSError("I2C bus not open")
